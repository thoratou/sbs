...
public class OptionProjects implements Option {
	...
	public OptionProjects() {
		option = "-p";
		configPath = new ArrayList<String>();
	}

	public OptionProjects(String defaultOption) {
	...
	public int perform(Parameters pars, int it) {
		...
		if(option.equals(par)){
			if(it+1<pars.size()){
				configPath.add(pars.getParameterAt(it+1));
				it++;
			}
			else{
	...
	public void usage(List<String> manUsage) {
		manUsage.add(option+" <component-path> : select specific project");		
	}
}